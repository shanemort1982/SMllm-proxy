...
async def fetch_available_models():
    ...
    if config.get("gemini_api_key"):
        try:
            ...
            # Gemini doesn't have a list models endpoint, so we use a predefined list
            gemini_models = [
    ...
    if model_config:
        ...
        logger.info(f"Updated model configuration with {len(model_config)} discovered models")
...
async def test_connection(
    ...
    try:
        ...
        if model.startswith("gpt") and not model_config.get("api_key"):
            return templates.TemplateResponse(
                "index.html", 
                {
                    "request": request,
        ...
        if model.startswith("gemini") and not model_config.get("api_key"):
            return templates.TemplateResponse(
                "index.html", 
                {
                    "request": request,
                    ...
                    "test_result": {
                        "status": "danger",
                        "message": "Gemini API Key must be configured"
                    }
                }
            )
        ...
        if not model_config.get("api_key"):
            logger.error(f"Connection test failed: {provider.capitalize()} API Key not configured")
            return templates.TemplateResponse(
                "index.html", 
                {
                    "request": request,
